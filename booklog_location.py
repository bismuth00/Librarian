    ...
    def build(self):
        ...
        def submit():
            with util.OpenWaitDialog(self.page, "書籍情報変更中…"):
                ...
                category_val = next(
                    (o.text for o in self.dropdown.options if o.key == category_key)
                )
                ...
                for result in util.process_text(process, self.text.value):
                    if is_successful(result):
                        book = result.unwrap()
                        next((c for c in self.categories if c["text"] == category_val))[
                            "count"
                        ] += 1
                        next(
                            (
                        ...
                        self.table.rows.insert(
                            ...
                            ft.DataRow(
                                cells=[
                                    ft.DataCell(
                                        ft.Text(
                                            len(self.table.rows) + 1, selectable=True
                                        )
                                    ),
                                    ft.DataCell(ft.Text(book["asin"], selectable=True)),
                                    ft.DataCell(
                                        ft.Text(book["title"], selectable=True)
                                    ),
                                    ft.DataCell(
                                        ft.Text(book["category"], selectable=True)
                                    ),
            ...
            self.page.update()
            if len(pending) > 0:
                util.OpenErrorDialog(
                    self.page, "{}件の変更に失敗しました".format(len(pending))
                )
