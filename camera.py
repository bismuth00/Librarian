...
def test_pyocr(e):
    ...
    roi_default = ((center_x - roi_size_x//2, center_y - roi_size_y//2), (center_x + roi_size_x//2, center_y + roi_size_y//2))
    ...
    while True:
        ...
        for box in txt:
            ...
            if not m: continue
            t = m.group(1).translate(str.maketrans({'-': None, '.': None, '}': '1', '{': '1'}))
        ...
        if position:
            ...
            width = int(width * 1.5)
            height = int(height * 2)
            roi_detect = ((center_x - width // 2, center_y - height // 2), (center_x + width // 2, center_y + height // 2))
